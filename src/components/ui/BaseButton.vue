<template>
  <button :type="type" :class="buttonClasses" :disabled="props.disabled || props.loading">
    ...
    <div v-else class="flex items-center justify-center gap-2 hover: ">
...
const buttonClasses = computed(() => {
  const classes = {
    'rounded-lg font-semibold cursor-pointer ': true,
    'flex items-center justify-center': props.loading,
    'opacity-50 !cursor-not-allowed ': props.disabled,
    'w-full': props.block,
    'bg-teal-500 text-white transition-bg duration-500 ease-in-out': props.variant === 'primary',
    'bg-gray-300 text-gray-800  transition-bg duration-500 ease-in-out': props.variant === 'secondary',
    'bg-rose-500 text-white transition-bg duration-500 ease-in-out': props.variant === 'danger',
